...
class CopilotService:
    ...
    async def draft_email(
        ...
        # Get lead context
        lead = await self._get_lead(request.lead_id)
        
        # Get contact if available
        contact = None
        if self._contact_repo:
            contact = await self._contact_repo.get_primary_for_lead(request.lead_id)
        
        # Get recent messages for context
        recent_messages = []
        if self._message_repo:
            messages = await self._message_repo.get_by_lead(
                request.lead_id, limit=5
            )
            recent_messages = messages
        
        # Build context for AI
        context = self._build_email_context(lead, contact, recent_messages, request)
        
        # Generate email (mock implementation - replace with actual AI)
        draft = await self._generate_email(context, request)
    ...
    async def _generate_email(
        ...
        if self._ai_client:
            result = await self._ai_client.generate(
                prompt=self._build_email_prompt(context, request),
        ...
        return EmailDraftResponse(
            subject=f"Following up - {request.purpose}",
            body=f"""Hi {first_name},

I hope this message finds you well. I wanted to reach out regarding {request.purpose}.
...
Best regards""",
            suggested_send_time=datetime.utcnow(),
    ...
    def _build_email_prompt(self, context: dict, request: EmailDraftRequest) -> str:
...
Generate a subject line and email body."""
    ...
    async def summarize_lead(
        ...
        contacts = []
        if self._contact_repo:
            result = await self._contact_repo.get_by_lead(
                request.lead_id, page=1, page_size=10
            )
            contacts = result.get("items", [])
        
        deals = []
        if request.include_deal_info and self._deal_repo:
            deals = await self._deal_repo.get_by_lead(request.lead_id)
        
        activity = []
        if request.include_activity_history and self._message_repo:
            activity = await self._message_repo.get_by_lead(
                request.lead_id, limit=20
            )
        
        # Generate summary
        summary = await self._generate_lead_summary(
            lead, contacts, deals, activity, request
        )
        
        # Log usage
        await self._log_usage(ctx, "summarize_lead", {
            "lead_id": str(request.lead_id)
        })
    ...
    async def suggest_next_steps(
        ...
        deal = None
        if request.deal_id and self._deal_repo:
            deal = await self._deal_repo.get_by_id(request.deal_id)
        
        # Get recent activity
        activity = []
        if self._message_repo:
            activity = await self._message_repo.get_by_lead(
                request.lead_id, limit=request.recent_activity_count
            )
        ...
        # Log usage
        await self._log_usage(ctx, "suggest_next_steps", {
            "lead_id": str(request.lead_id)
        })
    ...
    async def analyze_sentiment(
        ...
        if self._ai_client:
            result = await self._ai_client.analyze_sentiment(request.text)
    ...
    async def qualify_lead(
        ...
        criteria = request.qualification_criteria or {
        ...
        if self._contact_repo:
            contacts = await self._contact_repo.get_decision_makers(request.lead_id)
            if contacts:
