...
class ContactService(BaseService):
    ...
    async def get_contacts_for_lead(
        ...
        return ContactListResponse(
            items=[ContactResponse(**c) for c in result["items"]],
            total=result["total"],
            page=result["page"],
    ...
    async def get_primary_contact(
        ...
        return ContactResponse(**contact) if contact else None
    ...
    async def create_contact(
        ...
        if data.is_primary:
            await self._unset_primary_contacts(data.lead_id)
        
        # Create contact
        contact_data = data.model_dump()
        contact_data["lead_id"] = str(data.lead_id)
    ...
    async def update_contact(
        ...
        if data.is_primary is True and not contact.get("is_primary"):
