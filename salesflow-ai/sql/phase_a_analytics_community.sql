-- ====================================================================
-- SALES FLOW AI - PHASE A: ANALYTICS & COMMUNITY FOUNDATION
-- ====================================================================
-- Purpose: Die 3 fehlenden Komponenten für "World's Best AI Sales Tool"
-- 1. Success Stories (Social Proof)
-- 2. Template Performance Tracking (Analytics)
-- 3. Community Posts (Network Effects)
-- ====================================================================

-- ====================================================================
-- 1. COMPANY SUCCESS STORIES
-- ====================================================================
-- Social Proof & Testimonials pro Firma
-- Use Case: "Zeige mir Success Stories von Herbalife Partnern in DACH"

CREATE TABLE IF NOT EXISTS company_success_stories (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  company_name TEXT NOT NULL, -- Referenz zu network_marketing_companies
  user_id UUID, -- Optional: Referenz zu auth.users wenn User seine Story teilt
  ...
  timeframe_months INTEGER, -- Wie lange hat es gedauert?
  metrics JSONB, -- {"rank": "Diamond", "monthly_income": 5000, "team_size": 50}
  location_country TEXT, -- DE, AT, CH
  location_region TEXT, -- Bayern, Wien, Zürich
  verified BOOLEAN DEFAULT false, -- Admin-verifiziert?
  featured BOOLEAN DEFAULT false, -- Featured Story?
  image_url TEXT,
  video_url TEXT,
  tags TEXT[], -- ARRAY['success', 'team_building', 'fast_growth']
...
COMMENT ON TABLE company_success_stories IS 'Success Stories für Social Proof - zeigt was möglich ist';
COMMENT ON COLUMN company_success_stories.metrics IS 'JSON mit Achievement Details: rank, income, team_size, etc.';

-- ====================================================================
-- 2. TEMPLATE PERFORMANCE TRACKING
-- ====================================================================
-- DAS GEHIRN: Welche Templates funktionieren wirklich?
-- Use Case: "Zeige mir die besten Cold Outreach Templates für Herbalife"

CREATE TABLE IF NOT EXISTS template_performance (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  template_id TEXT NOT NULL, -- Referenz zu message templates
  company_name TEXT, -- Optional: Performance pro Firma
  user_id UUID, -- Wer hat es genutzt?
  ...
  times_positive_reply INTEGER DEFAULT 0, -- User markiert als "positiv"
  times_negative_reply INTEGER DEFAULT 0,
  times_converted INTEGER DEFAULT 0, -- Lead wurde zu Customer
  
  -- Calculated Metrics (auto-updated via triggers)
  delivery_rate DECIMAL(5,2), -- delivered / sent
  open_rate DECIMAL(5,2), -- opened / delivered
  response_rate DECIMAL(5,2), -- replied / opened
  conversion_rate DECIMAL(5,2), -- converted / replied
  
  -- Performance Score (AI-berechnet: 0-100)
  performance_score INTEGER CHECK (performance_score BETWEEN 0 AND 100),
  
  -- Context
  channel TEXT, -- whatsapp, email, dm
  target_audience TEXT, -- cold_market, warm_market, etc.
  language TEXT DEFAULT 'de',
  region TEXT, -- DACH, UK, US, LATAM
...
COMMENT ON TABLE template_performance IS 'Performance Tracking aller Message Templates - Das Gehirn der KI';
COMMENT ON COLUMN template_performance.performance_score IS 'AI-calculated: Kombination aus Response + Conversion + User Feedback';

-- ====================================================================
-- 3. COMMUNITY POSTS
-- ====================================================================
-- Network Effects: User helfen Usern
-- Use Case: "Best Practices teilen, Fragen stellen, Erfahrungen austauschen"

CREATE TABLE IF NOT EXISTS community_posts (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID NOT NULL, -- Wer hat gepostet?
  company_name TEXT, -- Optional: Firmen-spezifischer Post
  ...
  -- Kategorisierung
  category TEXT, -- 'objection_handling', 'recruiting', 'product_knowledge', 'mindset'
  tags TEXT[], -- ARRAY['herbalife', 'cold_calling', 'instagram']
  
  -- Firmen & Region
  relevant_companies TEXT[], -- Für welche Firmen relevant?
  relevant_regions TEXT[], -- DACH, UK, US, etc.
  language TEXT DEFAULT 'de',
  ...
  -- Moderation
  is_verified BOOLEAN DEFAULT false, -- Admin-verified als quality content
  ...
  attachments JSONB, -- PDFs, docs, etc.
...
COMMENT ON TABLE community_posts IS 'Community Posts für Network Effects und User Retention';
...
CREATE TABLE IF NOT EXISTS community_comments (
  ...
  parent_comment_id UUID REFERENCES community_comments(id), -- Für Replies
...
-- ====================================================================
-- 5. VIEWS FÜR ANALYTICS
-- ====================================================================

-- View: Top Performing Templates
CREATE OR REPLACE VIEW v_top_templates AS
SELECT 
  tp.template_id,
  tp.company_name,
  tp.channel,
...
WHERE tp.times_used >= 5 -- Mindestens 5x genutzt für Statistik
GROUP BY tp.template_id, tp.company_name, tp.channel, tp.target_audience
HAVING AVG(tp.performance_score) >= 60 -- Mindestens Score 60
ORDER BY avg_performance_score DESC, total_conversions DESC;

COMMENT ON VIEW v_top_templates IS 'Top Performing Templates für AI-Recommendations';

-- View: Company Success Stories Leaderboard
CREATE OR REPLACE VIEW v_success_leaderboard AS
SELECT 
  company_name,
...
COMMENT ON VIEW v_success_leaderboard IS 'Success Stories Ranking pro Firma';
...
-- ====================================================================
-- 6. FUNCTIONS FÜR AUTO-CALCULATIONS
-- ====================================================================

-- Function: Update Template Performance Rates
CREATE OR REPLACE FUNCTION update_template_performance_rates()
RETURNS TRIGGER AS $$
BEGIN
  -- Calculate rates
...
-- Function: Update Community Post Engagement
...
-- ====================================================================
-- 7. INITIAL DATA - EXAMPLE SUCCESS STORIES
-- ====================================================================

-- Beispiel Success Story: Zinzino
...
-- Beispiel Success Story: Herbalife
INSERT INTO company_success_stories (
  company_name, story_title, story_text, achievement_type,
