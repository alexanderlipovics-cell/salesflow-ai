...
export const TodayTasksCard = React.memo<TodayTasksCardProps>(
  ({ tasks, tasksCompleted, tasksOpen, isLoading, onTaskClick }) => {
    const columns = useMemo(
      () => [
        {
          key: "contact_name",
          header: "Kontakt",
          render: (value: string | null) => value || "Unbekannter Kontakt",
          sortable: true,
        },
        {
          key: "contact_status",
    ...
    return (
      <motion.div
        ...
        className="rounded-2xl border border-sf-border/80 bg-sf-card/95 shadow-lg"
      >
        <div className="flex flex-col gap-2 p-6 pb-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-sf-text-muted">
              Alle Tasks bis heute Nacht – ideal für dein Tages-Cockpit.
            </p>
          </div>
          <div className="text-xs text-sf-text-muted">
            <span className="font-medium text-sf-text">{tasksCompleted}</span> erledigt ·{" "}
            <span className="font-medium text-sf-text">{tasksOpen}</span> offen
          </div>
        </div>
        ...
        <div className="px-6 pb-6">
          <SFTable
            ...
            columns={columns}
            isLoading={isLoading}
            emptyMessage="Keine offenen Aufgaben für heute – dein Future-Ich sagt Danke. 🎉"
            onRowClick={(task) => onTaskClick?.((task as TodayTask).task_id)}
            keyExtractor={(task) => (task as TodayTask).task_id}
          />
        </div>
      </motion.div>
    );
