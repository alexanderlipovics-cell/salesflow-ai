-- Migration: Autopilot Engine V2 Tables
-- Date: 2025-01-06
-- Description: Tables for scheduling, rate limiting, A/B testing

-- ============================================================================
-- AUTOPILOT JOBS (Scheduled Messages)
-- ============================================================================
...
-- ============================================================================
-- A/B TEST RESULTS (Metrics Tracking)
-- ============================================================================
...
-- ============================================================================
-- CHANNEL CREDENTIALS (Secure Storage for API Keys)
-- ============================================================================
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM ab_test_experiments WHERE name = 'Objection Handling V1') THEN
        INSERT INTO ab_test_experiments (
            ...
            'Objection Handling V1',
            'Test different approaches to handling price objections',
            'active',
            '[
                {"id": "A", "name": "Empathetic", "template": "Ich verstehe total! Lass uns kurz sprechen - vielleicht finden wir eine Lösung."},
                {"id": "B", "name": "Direct", "template": "Alles klar! Lass mich dir zeigen wie andere das gelöst haben."},
                {"id": "C", "name": "Question", "template": "Interessant! Was genau stört dich am meisten?"}
            ]'::jsonb,
            '{"A": 0.33, "B": 0.33, "C": 0.34}'::jsonb,
            'reply_rate',
            'objection_handler',
            30,
...
-- SELECT cleanup_old_rate_limit_counters();
-- SELECT cleanup_old_autopilot_jobs();
...
--   AND table_name IN ('autopilot_jobs', 'rate_limit_counters', 'ab_test_experiments', 'ab_test_results', 'channel_credentials');
...
-- DROP FUNCTION IF EXISTS cleanup_old_rate_limit_counters CASCADE;
