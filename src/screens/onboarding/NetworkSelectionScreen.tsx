...
const NETWORKS: NetworkOption[] = [
  ...
  },
  {
    ...
    description: 'Ernährung, Gewichtsmanagement und Wellness-Produkte',
    features: ['Nutrition-Beratung', 'Gewichtsmanagement', 'Fitness-Coaching'],
    hasFullSupport: true,
  },
  {
...
export default function NetworkSelectionScreen({ navigation, onComplete }: Props) {
  ...
  const handleSelect = async () => {
    ...
    try {
      ...
      // 3. MLM Company in users table speichern
      const { error: mlmError } = await supabase
        .from('users')
        .update({
          mlm_company: selectedNetwork,
          mlm_rank: 'partner',
          mlm_rank_data: {},
        })
        .eq('id', user?.id);
      ...
      if (mlmError) {
        console.warn('Could not update MLM company:', mlmError);
                ...
                <View style={styles.cardHeader}>
                  ...
                  <View style={styles.cardTitleContainer}>
                    <View style={styles.cardTitleRow}>
                      ...
                      {network.hasFullSupport && (
                        <View style={[
                          styles.supportBadge,
                          selectedNetwork === network.slug && styles.supportBadgeSelected,
                        ]}>
                          <Text style={[
                            styles.supportBadgeText,
                            selectedNetwork === network.slug && styles.supportBadgeTextSelected,
                          ]}>
                            ✓ AI-optimiert
                          </Text>
                        </View>
