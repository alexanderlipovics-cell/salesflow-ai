...
async def create_message_event(
    ...
    Raises:
        ValueError: Wenn das Event nicht erstellt werden konnte
        Exception: Bei Datenbankfehlern (z.B. RLS, fehlende Tabelle)
    """
    logger.info(
    ...
    try:
        ...
        if not result.data:
            error_msg = "Fehler beim Erstellen des Message Events: Keine Daten zurückgegeben"
        ...
        # Re-raise ValueError (bereits geloggt)
        raise
    except Exception as e:
        # Detaillierte Fehlerbehandlung für verschiedene Fehlertypen
        error_str = str(e).lower()
        
        # RLS-Fehler oder fehlende Permissions
        if "permission denied" in error_str or "row-level security" in error_str or "policy" in error_str:
            logger.error(f"RLS/Permission error creating message event: {e}")
            raise ValueError(f"Keine Berechtigung zum Erstellen von Message Events. Bitte RLS-Policies prüfen.")
        
        # Tabelle existiert nicht
        if "does not exist" in error_str or "42p01" in error_str:
            logger.error(f"Table 'message_events' does not exist: {e}")
            raise ValueError(f"Tabelle 'message_events' existiert nicht. Bitte Migration ausführen.")
        
        # Andere Datenbankfehler
        logger.error(f"Database error creating message event: {e}")
        raise ValueError(f"Fehler beim Erstellen des Message Events: {str(e)}")
