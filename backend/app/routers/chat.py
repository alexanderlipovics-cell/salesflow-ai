...
async def chat_completion(
    ...
    # 1.3 Akquise-Gedächtnis: URL-Erkennung für Social Media Gruppen
    ...
    try:
        ...
        if urls:
            ...
            for url in urls:
                # Prüfe ob es eine Social Media Gruppe/Hashtag ist
                if memory.is_social_group_url(url):
                    ...
                    if existing:
                        ...
                        if is_exhausted:
                            akquise_reply = (
                                f"📍 **{source_name}**\n\n"
                                f"Du hast diese Quelle als 'erschöpft' markiert. "
                                f"Zuletzt genutzt: vor {days_ago} Tagen ({times_used}x insgesamt, {leads_found} Leads gefunden).\n\n"
                                f"Soll ich dir ähnliche Quellen vorschlagen?"
                            )
                        elif days_ago == 0:
                            akquise_reply = (
                                f"📍 **{source_name}**\n\n"
                        ...
                        akquise_reply = (
                            f"✅ Alles klar! Ich merke mir **{source_name}**.\n\n"
                            f"Los geht's! 🚀"
                        )
                        break
    except Exception as e:
        logger.debug(f"Akquise-Gedächtnis übersprungen: {e}")
    
    # Kurzschluss, wenn Akquise-Gedächtnis aktiv war
    if akquise_reply:
