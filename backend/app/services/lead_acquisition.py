...
class LeadAcquisitionService:
    ...
    async def acquire_lead(
        ...
        default_contact_status: Optional[str] = None,
    ) -> AcquisitionResult:
        """
        ...
        try:
            ...
            # 4. Lead erstellen (mit Kontext-Erkennung)
            lead_id, was_created = await self._create_lead(
                normalized, 
                user_id, 
            ...
            # 5. Follow-up planen wenn bereits kontaktiert
            if user_message_context:
                from app.utils.contact_status_detection import (
                    detect_contact_status_from_context,
                    should_schedule_followup,
                    get_followup_delay_days,
                )
                
                status_info = detect_contact_status_from_context(user_message_context)
                if should_schedule_followup(status_info) and was_created:
                    try:
                        delay_days = get_followup_delay_days(status_info)
                        # Follow-up über Follow-up-Engine planen
                        from app.services.followup_engine import FollowUpEngine
                        followup_engine = FollowUpEngine(self.db)
                        await followup_engine.schedule_followup(
                            lead_id=lead_id,
                            user_id=user_id or "",
                            days=delay_days,
                        )
                        logger.info(f"Follow-up geplant für Lead {lead_id} in {delay_days} Tagen")
                        result.warnings.append(f"Follow-up geplant in {delay_days} Tagen")
                    except Exception as e:
                        logger.warning(f"Konnte Follow-up nicht planen: {e}")
    ...
    async def _create_lead(
        ...
        """
        Erstellt neuen Lead in DB mit Duplikat-Check.
        
        Args:
            lead: Normalisierte Lead-Daten
            user_id: User ID
            default_contact_status: Vorgegebener Kontakt-Status
            user_message_context: User-Nachricht für Kontext-Erkennung
            
        Returns:
            Tuple[lead_id, was_created]
        """
        from app.utils.lead_duplicate_check import create_lead_if_not_exists
        from app.utils.contact_status_detection import detect_contact_status_from_context
        
        # Erkenne Kontakt-Status aus User-Nachricht (falls vorhanden)
        contact_status_info = None
        if user_message_context:
            contact_status_info = detect_contact_status_from_context(user_message_context)
            # Überschreibe default_contact_status wenn erkannt
            if contact_status_info.get('contact_status') != 'never_contacted':
                default_contact_status = contact_status_info.get('contact_status')
        ...
        data = {
            ...
            # Lead Status System
            "contact_status": default_contact_status or "never_contacted",
            "contact_count": contact_status_info.get('contact_count', 0) if contact_status_info else 0,
        }
        
        # Zusätzliche Felder wenn bereits kontaktiert
        if contact_status_info and contact_status_info.get('contact_status') == 'awaiting_reply':
            data["last_contact_date"] = contact_status_info.get('last_contact_date')
            data["awaiting_reply_since"] = contact_status_info.get('awaiting_reply_since')
        
        # Owner zuweisen falls vorhanden
        if user_id:
            data["user_id"] = user_id
        
        # Duplikat-Check und Erstellung
        created_lead, was_created = await create_lead_if_not_exists(
            db=self.db,
            lead_data=data,
            user_id=user_id or "",
        )
        ...
        if created_lead:
            ...
            # Wenn bereits kontaktiert und Follow-up nötig → Follow-up planen
            if contact_status_info and contact_status_info.get('needs_followup') and was_created:
                try:
                    from app.utils.contact_status_detection import get_followup_delay_days
                    from datetime import timedelta
                    
                    delay_days = get_followup_delay_days(contact_status_info)
                    due_at = datetime.utcnow() + timedelta(days=delay_days)
                    
                    # Follow-up direkt in follow_up_suggestions Tabelle erstellen
                    followup_data = {
                        "lead_id": lead_id,
                        "user_id": user_id or "",
                        "due_at": due_at.isoformat(),
                        "status": "pending",
                        "template_key": "fu_1_bump",  # Standard Follow-up Template
                        "note": f"Automatisch geplant nach Lead-Import (bereits kontaktiert)",
                        "created_at": datetime.utcnow().isoformat(),
                    }
                    
                    self.db.table("follow_up_suggestions").insert(followup_data).execute()
                    logger.info(f"Follow-up geplant für Lead {lead_id} in {delay_days} Tagen")
                except Exception as e:
                    logger.warning(f"Konnte Follow-up nicht planen: {e}")
            
            return lead_id, was_created
        else:
            raise Exception("Failed to create or find lead")
    ...
    async def import_from_csv(
        ...
        default_contact_status: str = "never_contacted",
    ) -> ImportResult:
        """
            ...
            for row_num, row in enumerate(rows, start=2):  # Start bei 2 wegen Header
                try:
                    ...
                    acquisition_result = await self.acquire_lead(
                        ...
                        default_contact_status=default_contact_status,
                    )
