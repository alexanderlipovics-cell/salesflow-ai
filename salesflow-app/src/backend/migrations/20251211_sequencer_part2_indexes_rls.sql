-- ╔════════════════════════════════════════════════════════════════════════════╗
-- ║  SEQUENCER ENGINE - PART 2: INDEXES & RLS                                  ║
-- ╚════════════════════════════════════════════════════════════════════════════╝
...
-- Helper Functions
CREATE OR REPLACE FUNCTION personalize_content(content TEXT, variables JSONB) RETURNS TEXT AS $$
DECLARE
    result TEXT := content;
    key TEXT;
    val TEXT;
BEGIN
    FOR key, val IN SELECT * FROM jsonb_each_text(variables)
    LOOP
        result := REPLACE(result, '{{' || key || '}}', COALESCE(val, ''));
    END LOOP;
    RETURN result;
END;
$$ LANGUAGE plpgsql;
