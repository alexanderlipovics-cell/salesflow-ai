...
export function usePhoenixCache(options: UsePhoenixCacheOptions = {}) {
  ...
  useEffect(() => {
    const unsubscribe = NetInfo.addEventListener((netState) => {
      ...
      // Wenn wieder online, Daten synchronisieren
  ...
  const refreshAllData = useCallback(async () => {
    const lastLocation = await AsyncStorage.getItem(CACHE_KEYS.LAST_LOCATION);
    if (lastLocation) {
  ...
  const clearCache = useCallback(async () => {
    await Promise.all([
      AsyncStorage.removeItem(CACHE_KEYS.NEARBY_LEADS),
      AsyncStorage.removeItem(CACHE_KEYS.IM_EARLY),
      AsyncStorage.removeItem(CACHE_KEYS.ACTIVE_SESSION),
    ]);
    setState({
      nearbyLeads: [],
      imEarlyData: null,
